...
def ejercicio1():
    ...
    if request.method == 'POST':  # Verifica si el formulario fue enviado
        ...
        totalSdescuento = tarros * precio_unitario  # Cálculo del precio total sin descuento

        # Aplicación de descuentos según la edad del usuario
        if 18 <= edad <= 30:
            descuento = totalSdescuento * 0.15  # Descuento del 15% para edades entre 18 y 30
        elif edad > 30:
            descuento = totalSdescuento * 0.25  # Descuento del 25% para mayores de 30 años
        else:
            descuento = 0  # Sin descuento para menores de 18


        total = totalSdescuento - descuento


        resultado = {
            'nombre': nombre,
            'totalSdescuento': totalSdescuento,
            'descuento': descuento,
            'total': total
        }
    # Renderiza la plantilla del ejercicio 1 con los resultados
    return render_template('ejercicio1.html', result=resultado)
